...
def test_low_risk_feature_uses_one_targeted_reviewer():
    assert reviewer_roles_for_ticket(_ticket("low")) == ("correctness_reviewer",)


def test_level_one_review_uses_only_the_primary_reviewer(monkeypatch):
    import codebot.review_config as review_config

    monkeypatch.setattr(
        review_config,
        "_config_instance",
        ReviewConfig(multi_reviewer_enabled=False),
    )

    assert reviewer_roles_for_ticket(_ticket("critical", "security")) == ("security_reviewer",)
...
def test_rework_with_a_plan_returns_directly_to_implementation(tmp_path: Path):
    ...
    assert rework_target_state(_ticket(), plans) in (TicketState.IMPLEMENT, TicketState.IMPLEMENTATION_READY, TicketState.IMPLEMENTING)
...
def test_review_speed_quality_summary(tmp_path: Path, monkeypatch):
    ...
    record_reviewer_metric("correctness_reviewer", "CB-1", "APPROVE", duration_s=2.0)
    record_reviewer_metric("correctness_reviewer", "CB-2", "REWORK", duration_s=6.0)
